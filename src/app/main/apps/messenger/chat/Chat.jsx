...
function Chat(props) {
	...
	const { setMainSidebarOpen, setEmployeeSidebarOpen } = useContext(ChatAppContext);
	const chatRef = useRef(null);
	const [message, setMessage] = useState('');
	const routeParams = useParams();
	const employeeId = routeParams.id;
	const { data: user } = useGetMessengerUserProfileQuery();
	const { data: chat } = useGetMessengerChatQuery(employeeId, {
		skip: !employeeId
	});
	const { data: selectedEmployee } = useGetMessengerEmployeeQuery(employeeId, {
		skip: !employeeId
	});
	...
	function isFirstMessageOfGroup(item, i) {
		return i === 0 || (chat[i - 1] && chat[i - 1].employeeId !== item.employeeId);
	}

	function isLastMessageOfGroup(item, i) {
		return i === chat.length - 1 || (chat[i + 1] && chat[i + 1].employeeId !== item.employeeId);
	}
				...
				<Toolbar className="flex items-center justify-between px-16 w-full">
					<div className="flex items-center">
						...
						<div
							...
							onKeyDown={() => setEmployeeSidebarOpen(true)}
							role="button"
							tabIndex={0}
						>
							<UserAvatar
									...
									return (
										<StyledMessageRow
											...
											className={clsx(
												...
												item.employeeId === user.id ? 'me' : 'employee',
