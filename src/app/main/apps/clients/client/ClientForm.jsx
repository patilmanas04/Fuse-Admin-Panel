...
import ClientEmailSelector from './email-selector/ClientEmailSelector';
 ...
 */
// Zod schema for ClientEmail
const ClientEmailSchema = z.object({
	email: z.string().optional(),
	type: z.string().optional()
});
// Zod schema for ClientPhoneNumber
const ClientPhoneNumberSchema = z.object({
...
const schema = z.object({
	...
	phoneNumbers: z.array(ClientPhoneNumberSchema).optional(),
...
function ClientForm() {
	...
	const { data: client, isError } = useGetClientsItemQuery(clientId, {
		skip: !clientId
	});
	const [createClient] = useCreateClientsItemMutation();
	const [updateClient] = useUpdateClientsItemMutation();
	const [deleteClient] = useDeleteClientsItemMutation();
	...
	function onSubmit(data) {
		if (clientId === 'new') {
			createClient({ client: data })
				.unwrap()
				.then((action) => {
					navigate(`/apps/clients/${action.id}`);
				});
		} else {
